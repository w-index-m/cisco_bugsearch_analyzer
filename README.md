...
- 🤖 **AI 分析** - Groq / Gemini / OpenRouter を使った発生可能性判定、長いリリースノートの要約、Excel出力への「AI解説（内容の解釈）」列自動追加（何が起きる不具合か・どんな環境で影響が出得るかを解説）
...
- **既知の問題の貼り付け解析**: Palo Alto の「Known and Addressed Issues」や YAMAHA のリリースノートなど、自動取得できない公式ページの本文をブラウザでコピーして貼り付けると、項目単位に分解してカテゴリ分け・日本語訳。「AI による可能性判定を使用」がONの場合、Cisco検索と同様に「AI解説（内容の解釈）」列も追加される
- **テキストを翻訳（単体ツール）**: バグ検索とは独立して、対応パーサーがまだ無いベンダーの文章などを一時的に日本語訳したい場合に使える汎用の翻訳ボックス
